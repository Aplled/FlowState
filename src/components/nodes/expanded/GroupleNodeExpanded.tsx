...
export function GroupleNodeExpanded({ node }: { node: FlowNode }) {
  ...
  const childNodes = allNodes.filter((n) => data.child_ids?.includes(n.id))
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="space-y-2">
        ...
        {childNodes.length === 0 ? (
          <p className="text-sm text-text-muted">No nodes in this group. Drag nodes into the group on the canvas.</p>
        ) : (
          <div className="space-y-1">
            {childNodes.map((n) => (
              <div key={n.id} className="flex items-center gap-2 px-3 py-1.5 rounded bg-bg-tertiary text-sm text-text">
                ...
                <span className="truncate">{(n.data as any)?.title || (n.data as any)?.label || n.type}</span>
              </div>
            ))}
          </div>
        )}
